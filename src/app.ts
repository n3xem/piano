...
class ThreeJSContainer {
    ...
    private createScene = () => {
        ...
        const WHITE_KEY_DEPTH = 10;
        const BLACK_KEY_DEPTH = 6;
        ...
        let white_key_geometry = new THREE.BoxGeometry(WHITE_KEY_WIDTH, 2, WHITE_KEY_DEPTH);
        white_key_geometry.name = "white";
        white_key_geometry.translate(0, 0, WHITE_KEY_DEPTH / 2);
        let black_key_geometry = new THREE.BoxGeometry(WHITE_KEY_WIDTH / 2, 2, 6);
        black_key_geometry.name = "black";
        black_key_geometry.translate(0, 0, BLACK_KEY_DEPTH / 2);
        ...
        let createKeyboard = (octave: number, x: number) => {
            let WHITE_KEY_SCALE: string[] = ["C", "D", "E", "F", "G", "A", "B"];
            let BLACK_KEY_SCALE: string[] = ["C#", "D#", "", "F#", "G#", "A#"];
            ...
            for (let i = 0; i < WHITE_KEY_NUM; i++) {
                ...
                tmp.position.set(i * (WHITE_KEY_WIDTH + WHITE_KEY_DISTANCE) + x, 0, - WHITE_KEY_DEPTH / 2);
                tmp.name = WHITE_KEY_SCALE[i] + octave;
                white_keys[i] = tmp;
                meshList.push(tmp);
            ...
            for (let i = 0; i < 6; i++) {
                ...
                tmp.position.set(1.05 + i * (WHITE_KEY_WIDTH + WHITE_KEY_DISTANCE) + x, 1, -1.7 - BLACK_KEY_DEPTH / 2);
                tmp.name = BLACK_KEY_SCALE[i] + octave;
                black_keys[i] = tmp;
                meshList.push(tmp);
        ...
        createKeyboard(4, -KEYBOARD_WIDTH - 0.1);
        createKeyboard(5, 0);
